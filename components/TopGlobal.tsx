...
const TopGlobal = (props: Props) => {
    ...
    return (
        <HomeSection title='Global top 200' children={
            <CardCarousel
                length={list.length}
                itemWidth={cardWidth}
                gap={gap}
                    ...
                    <Fragment>
                        {
                            list.map((_, i, a) => {
                                return (
                                    <MiniChartCard
                                        key={i}
                                        title='Easy on me' src={{ uri: 'https://is5-ssl.mzstatic.com/image/thumb/Music115/v4/73/6d/7c/736d7cfb-c79d-c9a9-4170-5e71d008dea1/886449666430.jpg/400x400cc.jpg' }} subtitle={`Adele-${i}`}
                                        style={{
                                            width: cardWidth,
